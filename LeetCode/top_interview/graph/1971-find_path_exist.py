...
#I tried using BFS, but it is not working
# class Solution:
        ...
        def dfs(curr):
            ...
            for nei in adj[curr]:
                if nei not in visited:
                    if dfs(nei):
                        return True
            # visited.remove(curr) # there is no need to remove the curr from visited 
            return False
        return dfs(source)


s = Solution()
edges = [[4,3],[1,4],[4,8],[1,7],[6,4],[4,2],[7,4],[4,0],[0,9],[5,4]]; n=10; source = 5; destination=9
...
n = 100; source = 20; destination = 53
print(s.validPath(n, edges, source, destination))
